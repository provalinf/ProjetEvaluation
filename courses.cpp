...
QSqlQueryModel *courses::getCoursDispo(int id_user) {
	QSqlQueryModel *res = new QSqlQueryModel();
	QSqlQuery *qry = new QSqlQuery();
	database->open();
	qry->prepare("SELECT * FROM courses WHERE valide = 1 AND "
						 ...
						 "date_fin>NOW() AND "
						 "(id_Registration!=(SELECT id_Registration FROM register_students WHERE id_Student=:idStudent) OR "
						 "NOT EXISTS (SELECT id_Registration FROM register_students WHERE id_Student=:idStudent)))");
	qry->bindValue(":idStudent", id_user);
	qry->exec();
	res->setQuery(*qry);
	database->close();
...
QSqlQueryModel *courses::getCoursProf(int id_user) {
	QSqlQueryModel *res = new QSqlQueryModel();
	QSqlQuery *qry = new QSqlQuery();
	database->open();
	qry->prepare(
			"SELECT id_course, c.name as 'Titre', d.name as 'Domaine', nb_place as 'Nb place', date_deb as 'Date de début', date_fin as 'Date de fin', "
					"CASE valide\n"
					"      WHEN '0' THEN 'Refusé'\n"
					"      WHEN '1' THEN 'Accepté'\n"
					"      ELSE 'Non contrôlé'\n"
	...
	qry->bindValue(":idAuthor", id_user);
	qry->exec();
	res->setQuery(*qry);
	database->close();
	return res;
...
QAbstractItemModel *courses::getCoursRessources(int id_cours) {
	...
	qry->prepare(
			"SELECT id_resource, title as 'Nom', tr.name as 'Type', descr as 'Description', date_deb as 'Date de début', date_fin as 'Date de fin' FROM resources as c "
					"JOIN Type_resource as tr ON tr.id_type=c.id_type "
					"WHERE id_course=:idCours");
